...
if __name__ == '__main__':
    ...
    mlu_image_style = image_style.mlu()
    net = VGG19().cpu()
    g_net = TransNet().cpu()
    ...
    s1, s2, s3, s4 = get_gram_matrix(s1).detach(), get_gram_matrix(s2).detach(), get_gram_matrix(s3).detach(), get_gram_matrix(s4).detach()
    j = 0
    count = 0
    epochs = 0
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            if i % 10 == 0:
                ...
                mlu_image_c = F.interpolate(mlu_image_c, size=mlu_image_g.shape[2:])
                save_image(torch.cat([mlu_image_c, mlu_image_g], dim=3), './out/train_mlu/{}.jpg'.format(count), normalize=True)
        j += 1
