...
if __name__ == '__main__':
    ...
    g_net.load_state_dict(torch.load('./models/fst.pth', map_location=torch.device('cpu')))
    ...
    for i, image in enumerate(data_group):
        ...
        image_g = F.interpolate(image_g, size=image_c.shape[2:])
        combined_image = torch.cat((image_c, image_g), dim=2)
        save_image(combined_image, './out/cpu/%d.jpg' % i)
