...
torch::Tensor active_sigmoid_mlu(torch::Tensor x) {
  ...
  //bang_sigmoid_sample(y_ptr, x_ptr, size);

  return y;
}

PYBIND11_MODULE(libmlu_custom_ext, m) {
  // TODO: 请补充pybind函数定义（python调用的函数名，函数指针）
  m.def("active_sigmoid_mlu", &active_sigmoid_mlu, "A function that get sigmoid", pybind11::arg("tensor"));
}
