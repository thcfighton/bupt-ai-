  ...
  function Run (nextThread: ptr to Thread)
    ...
    --
      ...
      while ! threadsToBeDestroyed.IsEmpty ()
        ...
        --threadManagerHoare.FreeThread (th)
      endWhile
...
--XXXXXXXXXXXXXXXXXXXXXX  MutexHoare XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
---XXXXXXXXXXXXXXXXXXXXXX  ConditionHoare  XXXXXXXXXXXXXXXXXXXXXXXXXXX
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- 
          var 
            ...
            -- Initialize Mutex and Condition variable OF the Monitor
      ...
      method GetANewThread () returns ptr to Thread
        ...
        var
          thread_p: ptr to Thread

          threadManagerLock.Lock()

          -- wait the "notEmpty" Condition
          while
            freeList.IsEmpty()
          ...
          thread_p = freeList.Remove()
          -- change its status to "JUST_CREATED"
          thread_p.status = JUST_CREATED
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- change its status to "UNUSED"
        th.status = UNUSED
        
        -- add to freeList
        freeList.AddToEnd(th)

        -- Signal a testing_thread which want a new thread
...
--XXXXXXXXXXXXXXXXXXXXXX  ThreadManagerHoare  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        var 
          process_p : ptr to ProcessControlBlock
          ...
          -- remove from freeList
          process_p = freeList.Remove()
          -- change its status to "ACTIVE"
          process_p.status = ACTIVE
          -- get a pid for the process
          process_p.pid = nextPid
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- change its status to "FREE"
        p.status = FREE

        -- add to freeList
        freeList.AddToEnd(p)

        -- signal a testing_process which want a new process
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
          ...
          -- wait for more frames being returned
          while (numberFreeFrames < numFramesNeeded)
            newFramesAvailable.Wait(&frameManagerLock)
          endWhile
          
          for i = 0 to numFramesNeeded - 1
            -- update framInUse the BitMap
            getting_frame = framesInUse.FindZeroAndSet()
            -- set the address of allocated frames 
            frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (getting_frame * PAGE_SIZE)
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          ...
          for i = 0 to numFramesReturned - 1
            ...
            location_in_BitMap = (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME ) / PAGE_SIZE
            framesInUse.ClearBit(location_in_BitMap)
...
----------------------  InitFirstProcess  ------------------------
  ...
  function InitFirstProcess ()
	 ...
	 t1 : ptr to Thread 
  	 ...
  	 t1 = threadManager.GetANewThread ()
	 t1.Init("UserProgram")
	 t1.Fork(StartUserProcess , 0)
...
------------------------ StartUserProcess  -------------------------  
  ...
  function StartUserProcess (arg: int)
	...
	if openfile == null 
		FatalError("File not found error")
	endIf

	initpc = openfile.LoadExecutable(addrSpace)
	if initpc < 0
		FatalError("Unrecoverable error.kernel process failed")
	endIf
	...
	addrSpace. SetToThisPageTable ()              --Init PTBR 
	currentThread.isUserThread = true             --Set isUserThread to true  
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      -- NOT IMPLEMENTED
      var 
	kernelBuffer : array [MAX_STRING_SIZE] of char			
	...
	a: int
      ...
      newAddrSpace.Init ()
      a = currentThread.myProcess.addrSpace.GetStringFromVirtual (&kernelBuffer, filename asInteger,MAX_STRING_SIZE)
      if a < 0
        return -1
      endIf
      
      openfile = fileManager.Open (&kernelBuffer)	
      if openfile == null
        return -1
      endIf
      ...
      initSystemStackTop = (&currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger
      oldStatus =  SetInterruptsTo (DISABLED)
      newAddrSpace. SetToThisPageTable ()	-- Step 2 : Init PTBR	
      currentThread.isUserThread = true		-- Step 3 : Set isUserThread to true
      BecomeUserThread(initUserStackTop, initpc, initSystemStackTop)
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
	...
	a = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
	if a < 0
	  return -1
	endIf 
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      printInt(fileDesc)
      nl()
      printPtr("Virtual Address of buffer =",buffer)
